...
const storage: Storage = typeof window !== 'undefined' && window.localStorage
  ? {
    ...
    setItem: (key: string, value: unknown) => Promise.resolve(window.localStorage.setItem(key, value)),
  ...
  : {
    ...
    setItem: async (key: string, value: unknown) => chrome.storage.local.set({ [key]: value }),
